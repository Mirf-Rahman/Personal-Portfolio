...
function validateEnvironment() {
  ...
  // Check for placeholder secret (only at runtime)
  if (betterAuthSecret === "build-time-placeholder-secret-must-be-32-chars-min") {
    throw new Error("Cannot use placeholder secret in production. Set BETTER_AUTH_SECRET or AUTH_JWT_SECRET.");
  }
}
...
// Call validation when module loads (server-side only)
ensureValidated();
