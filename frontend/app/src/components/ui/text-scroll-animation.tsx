...
export const ScrollCharacter = ({
  ...
  return (
    <motion.span
      className={cn(
        "inline-block transform-style-3d",
        isSpace && "w-2",
        className,
      )}
...
export const ScrollElement = ({
  ...
  return (
    <div ref={targetRef} className={className}>
      <motion.div style={{ y, opacity, scale, filter }}>{children}</motion.div>
    </div>
  );
};

// For grouped elements like lists where we want sequential stagger
export const StaggeredScrollElement = ({
  children,
  index,
  total,
  className,
  ...
  const end = Math.min(start + 0.4, 0.92); // Cap end so blur fully clears before progress hits 1
