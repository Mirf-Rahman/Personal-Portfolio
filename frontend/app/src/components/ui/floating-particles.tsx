...
export function FloatingParticles({
  ...
  useEffect(() => {
    ...
    const count1 = Math.floor(particleCount / 2)
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const range = Math.sqrt(Math.random()) * 250 // 0 to 250 radius, uniform
      ...
      if (i % 2 === 0) {
        positions[idx1 * 3] = x
        positions[idx1 * 3 + 1] = 1000
        positions[idx1 * 3 + 2] = z
        idx1++
      } else {
        positions2[idx2 * 3] = x
        positions2[idx2 * 3 + 1] = 1000
        positions2[idx2 * 3 + 2] = z
        idx2++
      }
    }
      ...
      for (let i = 0; i < movers.length; i++) {
        ...
        if (mover.is_active) {
          ...
          if (mover.position.y > 1000) {
            // Use uniform distribution here too for consistency
            const range = Math.sqrt(Math.random()) * 250
            const rad = getRadian(getRandomInt(0, 360))
            const x = Math.cos(rad) * range
            const z = Math.sin(rad) * range
        ...
        if (i % 2 === 0) {
          positionsArray[idx1 * 3] = mover.position.x
          positionsArray[idx1 * 3 + 1] = mover.position.y
          positionsArray[idx1 * 3 + 2] = mover.position.z
          idx1++
        } else {
          positionsArray2[idx2 * 3] = mover.position.x
          positionsArray2[idx2 * 3 + 1] = mover.position.y
          positionsArray2[idx2 * 3 + 2] = mover.position.z
          idx2++
        }
      }
