...
export async function PUT(
  ...
  try {
    ...
    const { title, description, technologies, imageUrl, liveUrl, githubUrl, featured, order, shouldSwap } = body;
    ...
    if (order !== undefined && shouldSwap === true) {
      const result = await db.transaction(async (tx) => {
        ...
        return { updatedProject: updatedProject[0], swappedProject: swappedProject[0] };
      });
